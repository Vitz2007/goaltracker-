...
struct AddGoalView: View {
    ...
    private func saveGoal() {
        if !newGoalTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            // This is the critical line merged from the other version
            // This now calls the new, improved function in NotificationManager
            NotificationManager.scheduleNotification(
                id: newGoal.id,
                title: newGoal.title,
                reminderDate: newGoal.reminderDate,
                reminderIsEnabled: newGoal.reminderIsEnabled
            )
